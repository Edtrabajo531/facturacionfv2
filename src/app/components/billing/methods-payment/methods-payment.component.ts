...
export class MethodsPaymentComponent implements OnInit {
  ...
  constructor(private toastrS: ToastrService, private modalS: NgbModal, public datePipe: DatePipe, private formBuilder: FormBuilder, private validatorsS: ValidatorsService, private DecimalP: DecimalPipe,) { }
  ...
  formC(pre: any = false) {
    ...
    this.form = this.formBuilder.group({
      ...
      amount: [this.newPay.amount, [Validators.required, Validators.max(999999999999), this.validatorsS.float]],
  ...
  resetNewPay() {
    ...
    this.newPay = { type: "CONTADO", amount: this.cashPaymentTotalPending(), id_formapago: 1, estarjeta: 0, esuninstrumentobanco: 0, cod_formapago: "01", nombre_formapago: 'EFECTIVO', fechalimite: this.now, dias: 1, p100interes_credito: 0, p100interes_mora: 0, cantidadcuotas: 1, abonoinicial: 0, montointerescredito: 0, date_of_issue: this.now, date_of_withdrawal: this.now, date: this.now };
    // <!-- fechalimite *
    // dias *
    // p100interes_credito*
    // p100interes_mora*
    ...
    if (index != undefined) {
      ...
      if (type == 'EFECTIVO') {
        ...
        if (this.edit.index == undefined) {
          ...
          typep = this.typesPayments.filter((item: any) => item.id_formapago == 1 && item.type == 'CONTADO');
        }
        ...
        if (this.edit.index == undefined) {
          ...
          typep = this.typesPayments.filter((item: any) => item.id_formapago == 1);
        }
      } else if (type == 'CHEQUE') {
        ...
        if (this.edit.index == undefined) {
          var vtype: any = 'CONTADO';
          typep = this.typesPayments.filter((item: any) => item.id_formapago == 2);
        }
        ...
        if (this.edit.index == undefined) {
          var vtype: any = 'CONTADO';
          typep = this.typesPayments.filter((item: any) => item.id_formapago == 3);
        }
        ...
        if (this.edit.index == undefined) {
          var vtype: any = 'CONTADO';
          typep = this.typesPayments.filter((item: any) => item.id_formapago == 4);
        }
      }
      ...
      if (this.edit.index == undefined) {
        ...
        this.edit = { index: undefined, type: vtype, amount: this.cashPaymentTotalPending(), id_formapago: typep[0].id_formapago, estarjeta: typep[0].estarjeta, esuninstrumentobanco: typep[0].esuninstrumentobanco, cod_formapago: typep[0].cod_formapago, nombre_formapago: typep[0].nombre_formapago, fechalimite: this.now, dias: 1, p100interes_credito: 0, p100interes_mora: 0, cantidadcuotas: 1, abonoinicial: 0, montointerescredito: 0, date_of_issue: this.now, date_of_withdrawal: this.now, date: this.now };
      }
      this.formC(true);
    } else {
      this.resetNewPay()
  ...
  showExtras() {
    ...
    if (id_formapago) {
      var typep = this.typesPayments.filter((item: any) => item.id_formapago == id_formapago);
    ...
    if (this.newPay.esuninstrumentobanco == 1 && this.newPay.type == 'CONTADO') {
      ...
      this.form?.get('date_of_issue')?.setValidators(this.dateValidators.concat(Validators.required));
      this.form?.get('date_of_issue')?.updateValueAndValidity();
      this.form?.get('date_of_withdrawal')?.setValidators(this.dateValidators.concat(Validators.required));
      ...
      this.form?.get('date')?.setValidators(this.dateValidators.concat(Validators.required));
      this.form?.get('date')?.updateValueAndValidity();
  ...
  submit() {
    ...
    let tp: any = this.payments.filter((item: any) => item.id_formapago == this.form?.get('id_formapago')?.value);
    ...
    if (this.edit.index != undefined) {
      ...
      if (this.newPay.type == 'CRÉDITO') {
        var exist = this.payments.filter((item: any, index: any) => item.type == 'CRÉDITO');
        if (exist.length != 0) {
        ...
        var exist = this.payments.filter((item: any, index: any) => item.id_formapago == this.newPay.id_formapago);
        console.log(this.newPay.id_formapago);

        if (exist.length != 0) {
          this.toastrS.warning('Ya existe un pago con forma de pago: "' + this.newPay.nombre_formapago + '"');
          this.sendToFatherF('hideLoader');
          return;
        }
      }
  ...
  cashPaymentTotalPending() {
    let total: any = 0;
    if (this.edit.index != undefined) {
      var payments = this.payments.filter((item: any, index: any) => index != this.edit.index);
    } else {
  ...
  deletePayment(indexv: any) {
    this.payments = this.payments.filter((item: any, index: any) => index != indexv);
    this.sendDataToFatherF(this.payments);
  }
  ...
  autoPayment(total: any) {
    ...
    if (length == 0 && total > 0) {
      var newp = { type: "CONTADO", amount: total, id_formapago: 1, estarjeta: 0, esuninstrumentobanco: 0, cod_formapago: "01", nombre_formapago: 'EFECTIVO', fechalimite: this.now, dias: 1, p100interes_credito: 0, p100interes_mora: 0, cantidadcuotas: 1, abonoinicial: 0, montointerescredito: 0, date_of_issue: this.now, date_of_withdrawal: this.now, date: this.now };
      console.log("new p");
